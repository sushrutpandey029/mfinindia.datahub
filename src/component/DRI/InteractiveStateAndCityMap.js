...
//             const baseUrl = "https://api.mfinindia.org/api/auth/new_fetch_records";
//             const url =
...
//                                         <b><br>MFIN-CGRM (as on <span class="record-value">${record.Date
...
//       d3.json(`/mapFiles/states/${state_Name}.json`)
//         .then(function (
//           citiesGeojson
//         ) {
...
//                   records.forEach((record) => {
//                     output += `<div class='map_data'>
//                                         <strong>${record.State_Name}, ${record.District_Name
//                       }</strong>
//                                        DRI Category: <span class="record-value">${record.DRI_Category
...
//                                         <b>MFIN-CGRM (as on <span class="record-value">${record.Date
//                       }</span>)</b> <br>
...
const InteractiveStateAndCityMap = () => {
  ...
  function renderStates(statesGeojson) {
    ...
    svg
      ...
      .attr("d", path)
      .on("mouseover", function (event, d) {
        tooltip.html("<div class='map_data'>Loading data...</div>");
        d3.select(this).style("fill", "#00aaff");
        tooltip.transition().duration(200).style("opacity", 1);
              ...
              if (records.length > 0) {
                ...
                records.forEach((record) => {
                  ...
                  output += `<div class='map_data'>
                    ...
                    DRI Scrore: <span class="record-value">${(
                      record.DRI_Score * 100
                    ).toFixed(2)}%</span><br>
                    GLP (Rs Cr): <span class="record-value">${formatNumberDigits(
                      record.GLP_RsCr
  ...
  useEffect(() => {
    ...
    // Clear any existing tooltips before loading new data
    d3.selectAll(".tooltip").remove();
    loadData();
    return () => {
      d3.selectAll(".tooltip").remove();
    };
  }, [districtColors, currentState]);
